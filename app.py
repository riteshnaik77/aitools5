...
def download_evaluation_pdf():
    """API endpoint to download resume evaluation results as a PDF report"""
    try:
        ...
        job_title = (meta.get('job_title') or evaluation.get('job_title') or '').strip()
        oorwin_job_id = (meta.get('oorwin_job_id') or evaluation.get('oorwin_job_id') or '').strip()
        candidate_filename = (meta.get('candidate_filename') or evaluation.get('filename') or '').strip()
        evaluated_at_raw = meta.get('evaluated_at') or evaluation.get('evaluated_at')

        evaluated_display = ''
        if evaluated_at_raw:
            try:
                evaluated_dt = datetime.fromisoformat(evaluated_at_raw.replace('Z', '+00:00'))
                evaluated_display = evaluated_dt.strftime('%d %b %Y, %I:%M %p')
            except ValueError:
                evaluated_display = evaluated_at_raw
        else:
            evaluated_display = datetime.utcnow().strftime('%d %b %Y, %I:%M %p')

        match_percentage = evaluation.get('match_percentage')
        match_percentage_str = evaluation.get('match_percentage_str')
        if not match_percentage_str and match_percentage is not None:
            match_percentage_str = f"{match_percentage}%"
        ...
        if candidate_filename:
            meta_rows.append(
                [Paragraph('<b>Candidate</b>', table_header_style), Paragraph(sanitize_text(candidate_filename), body_style)]
            )
        if job_title:
            meta_rows.append(
                [Paragraph('<b>Job Title</b>', table_header_style), Paragraph(sanitize_text(job_title), body_style)]
            )
        if oorwin_job_id:
            meta_rows.append(
                [Paragraph('<b>Job ID</b>', table_header_style), Paragraph(sanitize_text(oorwin_job_id), body_style)]
            )
        meta_rows.append(
            [Paragraph('<b>Evaluated On</b>', table_header_style), Paragraph(sanitize_text(evaluated_display), body_style)]
        )

        meta_table = Table(meta_rows, colWidths=[120, available_width - 120])
        ...
        if match_percentage_str:
            elements.append(Spacer(1, 0.2 * inch))
            elements.append(Paragraph(f"<b>Overall Match Score:</b> {sanitize_text(match_percentage_str)}", section_heading_style))

        match_factors = evaluation.get('match_factors') or {}
        if match_factors:
            elements.append(Spacer(1, 0.1 * inch))
            elements.append(Paragraph('Match Factor Breakdown', subheading_style))
            factor_order = [
                'Skills Match',
                'Experience Match',
                'Education Match',
            ...
            seen = set()
            rows = [[Paragraph('Factor', table_header_style), Paragraph('Score', table_header_style)]]
            for key in factor_order:
                if key in match_factors:
                    value = match_factors.get(key)
                    if value is None:
                        display_value = 'Not Applicable'
                    else:
                        display_value = f"{value}%" if isinstance(value, (int, float)) else str(value)
                    rows.append([
                        Paragraph(sanitize_text(key), table_body_style),
                        Paragraph(sanitize_text(display_value), table_body_style)
                    ])
                    seen.add(key)

            # Include any additional dynamic factors
            for key, value in match_factors.items():
                if key in seen:
                    continue
                display_value = 'Not Applicable' if value is None else (f"{value}%" if isinstance(value, (int, float)) else str(value))
                rows.append([
                    Paragraph(sanitize_text(key), table_body_style),
                    Paragraph(sanitize_text(display_value), table_body_style)
                ])

            factor_table = Table(rows, colWidths=[available_width * 0.5, available_width * 0.5])
            factor_table.setStyle(TableStyle([
                ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#2c3e50')),
                ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
                ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
                ('VALIGN', (0, 0), (-1, -1), 'TOP'),
                ('GRID', (0, 0), (-1, -1), 0.4, colors.HexColor('#34495e')),
                ('ROWBACKGROUNDS', (0, 1), (-1, -1), [colors.white, colors.HexColor('#f8f9fa')]),
                ('LEFTPADDING', (0, 0), (-1, -1), 6),
                ('RIGHTPADDING', (0, 0), (-1, -1), 6),
                ('TOPPADDING', (0, 0), (-1, -1), 4),
        ...
        if profile_summary:
            elements.append(Spacer(1, 0.2 * inch))
            elements.append(Paragraph('Profile Summary', section_heading_style))
            elements.append(Paragraph(sanitize_text(profile_summary), body_style))

        over_under = evaluation.get('over_under_qualification')
        if over_under:
            elements.append(Paragraph('Qualification Fit Note', subheading_style))
            elements.append(Paragraph(sanitize_text(over_under), body_style))

        missing_keywords = evaluation.get('missing_keywords')
        elements.append(Spacer(1, 0.1 * inch))
        elements.append(Paragraph('Missing Keywords', section_heading_style))
        if missing_keywords:
            for keyword in missing_keywords:
                elements.append(Paragraph(f'• {sanitize_text(keyword)}', bullet_style))
        else:
            elements.append(Paragraph('None identified. Candidate covers the requested keywords.', body_style))

        fit_analysis = evaluation.get('candidate_fit_analysis') or {}
        if fit_analysis:
            elements.append(Spacer(1, 0.2 * inch))
            elements.append(Paragraph('Candidate Fit Analysis', section_heading_style))

            dimension_eval = fit_analysis.get('Dimension Evaluation') or []
            if dimension_eval:
                elements.append(Paragraph('Dimension Evaluation', subheading_style))
                rows = [[Paragraph('Dimension', table_header_style),
                         Paragraph('Evaluation', table_header_style),
                         Paragraph('Recruiter Comments', table_header_style)]]
                for entry in dimension_eval:
                    rows.append([
                        Paragraph(sanitize_text(entry.get('Dimension', '')), table_body_style),
                        Paragraph(sanitize_text(entry.get('Evaluation', '')), table_body_style),
                        Paragraph(sanitize_text(entry.get('Recruiter Comments', '')), table_body_style)
                    ])
                dim_table = Table(rows, colWidths=[available_width * 0.25, available_width * 0.25, available_width * 0.5])
            ...
            risks = fit_analysis.get('Risk and Gaps') or []
            elements.append(Spacer(1, 0.05 * inch))
            elements.append(Paragraph('Risk & Gap Checklist', subheading_style))
            if risks:
                rows = [[Paragraph('Area', table_header_style),
                         Paragraph('Risk', table_header_style),
                         Paragraph('Recruiter Strategy', table_header_style)]]
                for risk in risks:
                    rows.append([
                        Paragraph(sanitize_text(risk.get('Area', '')), table_body_style),
                        Paragraph(sanitize_text(risk.get('Risk', '')), table_body_style),
                        Paragraph(sanitize_text(risk.get('Recruiter Strategy', '')), table_body_style)
                    ])
                risk_table = Table(rows, colWidths=[available_width * 0.25, available_width * 0.35, available_width * 0.4])
                risk_table.setStyle(TableStyle([
                    ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#8e44ad')),
                    ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
                    ('GRID', (0, 0), (-1, -1), 0.4, colors.HexColor('#5e3370')),
                ...
                elements.append(Paragraph('No major risks flagged.', body_style))

            recommendation = fit_analysis.get('Recommendation')
            if recommendation:
                elements.append(Spacer(1, 0.05 * inch))
                elements.append(Paragraph('Recommendation', subheading_style))
                rec_rows = [[Paragraph('Verdict', table_header_style),
                             Paragraph('Fit Level', table_header_style),
                             Paragraph('Rationale', table_header_style)]]
                rec_rows.append([
                    Paragraph(sanitize_text(recommendation.get('Verdict', '')), table_body_style),
                    Paragraph(sanitize_text(recommendation.get('Fit Level', '')), table_body_style),
                    Paragraph(sanitize_text(recommendation.get('Rationale', '')), table_body_style)
                ])
                rec_table = Table(rec_rows, colWidths=[available_width * 0.2, available_width * 0.2, available_width * 0.6])
                rec_table.setStyle(TableStyle([
                    ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#1abc9c')),
                    ('TEXTCOLOR', (0, 0), (-1, 0), colors.white),
                    ('GRID', (0, 0), (-1, -1), 0.4, colors.HexColor('#16a085')),
                    ('VALIGN', (0, 0), (-1, -1), 'TOP'),
                    ('LEFTPADDING', (0, 0), (-1, -1), 5),
                    ('RIGHTPADDING', (0, 0), (-1, -1), 5),
                    ('TOPPADDING', (0, 0), (-1, -1), 4),
            ...
            recruiter_narrative = fit_analysis.get('Recruiter Narrative')
            if recruiter_narrative:
                elements.append(Paragraph('Recruiter Narrative', subheading_style))
                elements.append(Paragraph(sanitize_text(recruiter_narrative), mini_style))

        job_stability = evaluation.get('job_stability') or {}
        if job_stability:
            elements.append(Spacer(1, 0.2 * inch))
            elements.append(Paragraph('Job Stability Snapshot', section_heading_style))
            stability_rows = []
            for label, key in [
                ('Stability Score', 'StabilityScore'),
                ('Risk Level', 'RiskLevel'),
                ('Average Tenure', 'AverageJobTenure'),
                ('Job Count', 'JobCount')
            ]:
                value = job_stability.get(key)
                if value is not None:
                    stability_rows.append([
                        Paragraph(f'<b>{sanitize_text(label)}</b>', body_style),
                        Paragraph(sanitize_text(value), body_style)
                    ])

            if stability_rows:
                stability_table = Table(stability_rows, colWidths=[available_width * 0.4, available_width * 0.6])
                stability_table.setStyle(TableStyle([
                    ('BOX', (0, 0), (-1, -1), 0.4, colors.HexColor('#95a5a6')),
            ...
            reasoning = job_stability.get('ReasoningExplanation')
            if reasoning:
                elements.append(Paragraph('Stability Notes', subheading_style))
                elements.append(Paragraph(sanitize_text(reasoning), mini_style))

        career_progression = evaluation.get('career_progression') or {}
        if career_progression:
            elements.append(Spacer(1, 0.2 * inch))
            elements.append(Paragraph('Career Progression Insights', section_heading_style))

            progression_score = career_progression.get('progression_score')
            if progression_score is not None:
                elements.append(Paragraph(f'Progression Score: {sanitize_text(progression_score)}', body_style))

            key_obs = career_progression.get('key_observations') or []
            if key_obs:
                elements.append(Paragraph('Key Observations', subheading_style))
                for item in key_obs:
                    elements.append(Paragraph(f'• {sanitize_text(item)}', bullet_style))

            red_flags = career_progression.get('red_flags') or []
            elements.append(Paragraph('Red Flags', subheading_style))
            if red_flags:
                for flag in red_flags:
                    elements.append(Paragraph(f'• {sanitize_text(flag)}', bullet_style))
            else:
                elements.append(Paragraph('No major red flags detected.', body_style))

        # Interview question sections
        def render_question_block(title, questions):
            elements.append(Spacer(1, 0.1 * inch))
            elements.append(Paragraph(title, section_heading_style))
            if questions:
                for question in questions:
                    elements.append(Paragraph(f'• {sanitize_text(question)}', bullet_style))
            else:
                elements.append(Paragraph('No questions available for this section.', body_style))

        render_question_block('Behavioral / Quick Check Questions', evaluation.get('behavioral_questions') or [])
        render_question_block('Soft Skill Questions', evaluation.get('nontechnical_questions') or [])
        render_question_block('Technical Questions', evaluation.get('technical_questions') or [])

        def _set_meta(canvas, _):
            title_parts = [p for p in ["Resume Evaluation", candidate_filename or job_title, oorwin_job_id] if p]
            canvas.setTitle(" - ".join(title_parts))
            canvas.setAuthor("PeopleLogic PeopleBot")
        ...
        safe_candidate = re.sub(r'[^A-Za-z0-9 _\-]+', '', candidate_filename) if candidate_filename else ''
        safe_job = re.sub(r'[^A-Za-z0-9 _\-]+', '', oorwin_job_id) if oorwin_job_id else ''
        safe_title = re.sub(r'[^A-Za-z0-9 _\-]+', '', job_title) if job_title else ''

        filename_parts = ['Evaluation']
        if safe_candidate:
            filename_parts.append(safe_candidate)
        elif safe_title:
            filename_parts.append(safe_title)
        if safe_job:
            filename_parts.append(safe_job)

        filename = "_".join(filename_parts) if len(filename_parts) > 1 else 'Evaluation_Report'
        response.headers['Content-Disposition'] = f'attachment; filename="{filename}.pdf"'
        return response

    except Exception as e:
        logging.error(f"Error generating evaluation PDF: {str(e)}")
        return jsonify({
            'success': False,
            'message': str(e)
        }), 500
